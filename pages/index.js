...
/**
 * Programmatically calculates Left/Right turns based on relative vectors and a consistent cyclic rule.
 * L/R is determined by matching axis and direction (sign) based on the incoming vector.
 */
const calculateRelativeTurns = () => {
  ...
  for (let current = 0; current < 8; current++) {
    ...
    for (const previous of ADJACENCY[current]) {
      ...
      let incoming_axis_idx = -1; // 0 for X, 1 for Y, 2 for Z
      let incoming_dir_sign = 0;  // +1 for positive, -1 for negative

      if (incoming_vec[0] !== 0) { incoming_axis_idx = 0; incoming_dir_sign = Math.sign(incoming_vec[0]); }
      else if (incoming_vec[1] !== 0) { incoming_axis_idx = 1; incoming_dir_sign = Math.sign(incoming_vec[1]); }
      else if (incoming_vec[2] !== 0) { incoming_axis_idx = 2; incoming_dir_sign = Math.sign(incoming_vec[2]); }

      // Defines the cyclic order for determining Left/Right turn axes
      // e.g., if incoming is X (0), Left is Y (1), Right is Z (2)
      const turnAxisCycle = [
          [1, 2], 
          [2, 0], 
          [0, 1]  
      ...
      const [leftTurnAxisIdx, rightTurnAxisIdx] = turnAxisCycle[incoming_axis_idx];

      // Helper to get the target node for a given axis and direction
      const getTurnNode = (fromCoords, turnAxis, turnSign) => {
          ...
          return -1; // Should not happen for a valid cube turn
      };

      const Left_node = getTurnNode(C_coords, leftTurnAxisIdx, incoming_dir_sign);
      const Right_node = getTurnNode(C_coords, rightTurnAxisIdx, incoming_dir_sign);
